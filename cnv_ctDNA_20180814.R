...
if (TRUE){
  ...
  source("/home/wqh/workdir/src/ExomeCopy.R")
}
...
if(length(out) != 0){
  ...
  write.table(Out_colnames,sprintf("%s/%s.CNV.xls",opts$outdir,opts$sample),quote = F,sep = "\t",col.names = F,row.names = F)
  write.table(Out,sprintf("%s/%s.CNV.xls",opts$outdir,opts$sample),quote = F,col.names = F,sep = "\t",row.names = F,append = T)
}else{
...
if(length(out_raw) != 0){
  ...
  Out_Raw_colnames = colnames(Out)
  Out_Raw_colnames[1] = "#Sample" 
  Out_Raw_colnames = rbind(Out_Raw_colnames)
  write.table(Out_Raw_colnames,sprintf("%s/%s.CNV_raw.xls",opts$outdir,opts$sample),quote = F,sep = "\t",col.names = F,row.names = F)
...
all_out = DataFrame(sample = opts$sample,chromosome =counts@seqnames,ranges = counts@ranges,gene = counts$gene, copy.num = counts$copy_num,copy.num.raw = counts$copy_num_raw)
