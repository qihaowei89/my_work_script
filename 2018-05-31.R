# source("src/biocLite.R")
# install.packages("BiocInstaller", repos="http://bioconductor.org/packages/3.6/bioc")
# install.packages("Cairo")
biocLite("exomeCopy")

"exomeCopy包的设计目的是解决以下情况:
1 target富集，例如exome enrichment，导致非均匀read depth.
2 CNVs重叠富集区域可被检测为相对于背景read depth(通过control 集的平均值来生成)增加或减少
3 samples 或多或少与背景read depth相关,GC含量不同
how to :
  1) import experiment data, 
  2) generate background read depth, 
  3) recover CNVs using exomeCopy
R package ReadDepth can be used for CNV detection in whole genome(not appropriate for cancer exome sequencing--amplifications can be high)
R package exomeCNV is designed for tumor/normal paired exome sequencing data.
"
...
###################################################
library(exomeCopy)
gr <- GRanges(seqname="seq1",IRanges(start=1,end=249250621))
subdivideGRanges(gr,subsize = 5000)
...
target.sub <- subdivideGRanges(target,subsize = 30)
target.sub
...
" 
Steps: 
  1. given a vector of names of samples to be used as background, extract the read counts data frame from the GRanges object
  2. divide each sample by its mean read count (column means)
  3. calculate the median of these normalized read counts (row medians)
Note:
  Note that if the control set is a mix of males and females, the allosome portion of chrX will
  be a mixture of copy counts 1 and 2. In this case, one should created separate GRanges objects
  for chrX and chrY and run males and females separately with the appropriate expected copy
  count
"
chr1a <- exomecounts
...
# library(magrittr)
# a = example.counts %>% mcols() %>% '['(-1)
# b = apply(a,2, mean)
# for(i in 1:dim(a)[2]){
#   a[i]=a[[i]]/b[[i]]
# }
# a$median = apply(a, 1, median )
# head(a)
# head(example.counts)
...
for (i in 1:8) {
  mcols(example.counts)[[exome.samples[i]]] <- simulateCNV(x=mcols(example.counts)[[exome.samples[i]]],indices=cnv.starts[i]:(cnv.starts[i] + 99),multiply=cnv.mult[i], prob=cnv.probs[i])
  mcols(example.counts)[[exome.samples[i]]] <- simulateCNV(x=mcols(example.counts)[[exome.samples[i]]],indices=1000 + cnv.starts[i]:(cnv.starts[i] + 99),multiply=cnv.mult[i],prob=cnv.probs[i])
}
